...
void nrf_config::send_profile_toSCP () {
        //Get IP Addresses for Remote Servers
        const char *tmp1 = getenv("WAVE_L_IP");
        std::string WLIP(tmp1 ? tmp1 : "");
        if (WLIP.empty()) {
                Logger::nrf_app().error("WAVE local server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::nrf_app().info("WAVE local server IP is : %s", WLIP.c_str());

        const char *tmp2 = getenv("SMF_IP");
        std::string SMF_IP(tmp2 ? tmp2 : "");
        if (SMF_IP.empty()) {
                Logger::nrf_app().error("SMF WAVE-SCP server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::nrf_app().info("SMF WAVE-SCP server IP is : %s", SMF_IP.c_str());

        const char *tmp3 = getenv("WAVE_SCP_PORT");
        std::string WSPORT(tmp3 ? tmp3 : "");
        if (WSPORT.empty()) {
                Logger::nrf_app().error("WAVE-SCP server port not found");
                exit(EXIT_FAILURE);
        }
        Logger::nrf_app().info("WAVE remote server port is : %s", WSPORT.c_str());

        std::string uriwl = "http://" + WLIP + ":" + WSPORT + "/oai/discoverdata";
        std::string wl_send = std::string("NRF") + "&" + std::string(inet_ntoa(sbi.addr4)) + "&" + std::string("SMF") + "&" + std::string(SMF_IP.c_str());
        std::string wl_resp;

        Logger::nrf_app().info("Sending the profile information to the Local WAVE SCP");

        send_custom_curl(uriwl, wl_send, wl_resp);

        Logger::nrf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());
}
//----------------------------------------------------------------------------
