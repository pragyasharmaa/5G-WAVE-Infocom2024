...
void smf_config::send_profile_toSCP () {
        //Get IP Addresses for Remote Servers
        const char *tmp1 = getenv("WAVE_L_IP");
        std::string WLIP(tmp1 ? tmp1 : "");
        if (WLIP.empty()) {
                Logger::smf_app().error("WAVE local server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::smf_app().info("WAVE local server IP is : %s", WLIP.c_str());

        const char *tmp2 = getenv("WAVE_R_IP");
        std::string WRIP(tmp2 ? tmp2 : "");
        if (WRIP.empty()) {
                Logger::smf_app().error("WAVE-SCP server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::smf_app().info("WAVE-SCP server IP is : %s", WRIP.c_str());

        const char *tmp3 = getenv("WAVE_SCP_PORT");
        std::string WSPORT(tmp3 ? tmp3 : "");
        if (WSPORT.empty()) {
                Logger::smf_app().error("WAVE-SCP server port not found");
                exit(EXIT_FAILURE);
        }
        Logger::smf_app().info("WAVE remote server port is : %s", WSPORT.c_str());

        std::string uriwl = "http://" + WLIP + ":" + WSPORT + "/oai/discoverdata";
        std::string wl_send_1 = std::string("SMF") + "&" + std::string(inet_ntoa(n4.addr4)) + "&" + std::string("NRF") + "&" + std::string(inet_ntoa(nrf_addr.ipv4_addr));
        std::string wl_send_2 = std::string("SMF") + "&" + std::string(inet_ntoa(n4.addr4)) + "&" + std::string("AMF") + "&" + std::string(inet_ntoa(amf_addr.ipv4_addr));

        std::string wl_resp;

        Logger::smf_app().info("Sending the profile information to the Local WAVE SCP");

        send_custom_curl(uriwl, wl_send_1, wl_resp);
        Logger::smf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());
