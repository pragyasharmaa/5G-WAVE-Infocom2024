...
void amf_config::send_profile_toSCP () {
        //Get IP Addresses for Remote Servers
        const char *tmp1 = getenv("WAVE_L_IP");
        std::string WLIP(tmp1 ? tmp1 : "");
        if (WLIP.empty()) {
                Logger::amf_app().error("WAVE local server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::amf_app().info("WAVE local server IP is : %s", WLIP.c_str());

        const char *tmp2 = getenv("SMF_IPV4_ADDR_0");
        std::string SMF_IP(tmp2 ? tmp2 : "");
        if (SMF_IP.empty()) {
                Logger::amf_app().error("SMF WAVE-SCP server IP not found");
                exit(EXIT_FAILURE);
        }
        Logger::amf_app().info("SMF WAVE-SCP server IP is : %s", SMF_IP.c_str());

        const char *tmp4 = getenv("WAVE_SCP_PORT");
        std::string WSPORT(tmp4 ? tmp4 : "");
        if (WSPORT.empty()) {
                Logger::amf_app().error("WAVE-SCP server port not found");
                exit(EXIT_FAILURE);
        }
        Logger::amf_app().info("WAVE remote server port is : %s", WSPORT.c_str());

        std::string uriwl = "http://" + WLIP + ":" + WSPORT + "/oai/discoverdata";
        std::string wl_send_1 = std::string("AMF") + "&" + std::string(inet_ntoa(n2.addr4)) + "&" + std::string("NRF") + "&" + std::string(inet_ntoa(nrf_addr.ipv4_addr));
        //std::string wl_send_2 = std::string("AMF") + "&" + std::string(inet_ntoa(n2.addr4)) + "&" + std::string("UDM") + "&" + std::string(inet_ntoa(udm_addr.ipv4_addr));
        std::string wl_send_3 = std::string("AMF") + "&" + std::string(inet_ntoa(n2.addr4)) + "&" + std::string("AUSF") + "&" + std::string(inet_ntoa(ausf_addr.ipv4_addr));
        std::string wl_send_4 = std::string("AMF") + "&" + std::string(inet_ntoa(n2.addr4)) + "&" + std::string("SMF") + "&" + std::string(SMF_IP.c_str());
        
        std::string wl_resp;

        Logger::amf_app().info("Sending the profile information to the Local WAVE SCP");

        send_custom_curl(uriwl, wl_send_1, wl_resp);
        Logger::amf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());

        //send_custom_curl(uriwl, wl_send_2, wl_resp);
        //Logger::amf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());

        send_custom_curl(uriwl, wl_send_3, wl_resp);
        Logger::amf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());

        send_custom_curl(uriwl, wl_send_4, wl_resp);
        Logger::amf_app().info("Received response from Local WAVE SCP: %s", wl_resp.c_str());
}
//----------------------------------------------------------------------------
