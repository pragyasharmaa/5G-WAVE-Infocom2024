...
folder=sys.argv[1]
path = folder + "/*-rp.log"
path_wavelogs = folder + "/*-wave.log"

logFiles = glob.glob(path)
wavelogFiles = glob.glob(path_wavelogs)
...
for filename in logFiles:
    ...
    if not df_all.empty:
        ...
        df_min = df_all[['type','client','host']].copy()
        # print(df_min)

        csv_file = folder + '/' + vnf_name.replace('-rp','') + '-wave.csv'  
        df_min.to_csv(csv_file)            

#------------------------------------------------------------------------------------

path = folder + "/*-wave.log"
logfiles = glob.glob(path)
verification_list = []

for filename in logfiles:
    vnf_name = filename.split('/')[1].split('.')[0]
    csvpath = folder + '/' + vnf_name + '.csv'
    ...
    for line in file.readlines():
        if "Verification" in line:
            ...
            verf_times.append(int(time))
    df['verify'] = verf_times
    # print(df)
    df.to_csv(csvpath)

#------------------------------------------------------------------------------------

path = folder + "/*-wave.csv"
...
for filename in csvFiles:
    ...
    for line in file.readlines():
        ...
        if vnf_to == "nrf" and reqtype == "PATCH":
            ...
            if vnf_to == "nrf" and vnf_from == "amf":
                req['id'] = 1
            elif vnf_to == "nrf" and vnf_from == "smf":
                if reqtype == "POST":
                    req['id'] = 2
                else:
                    req['id'] = 3
            elif vnf_to == "ausf" and vnf_from == "amf":
                if reqtype == "POST":
                    req['id'] = 4
                else:
                    req['id'] = 8
            elif vnf_to == "udm" and vnf_from == "ausf":
                if int(seqno) == 0:
                    req['id'] = 5
                else:
                    req['id'] = 9
            elif vnf_to == "udr" and vnf_from == "udm":
                if int(seqno) == 0:
                    req['id'] = 6
                elif int(seqno) == 1:
                    req['id'] = 7
                    ...
                    req['id'] = 11
            elif vnf_to == "smf" and vnf_from == "amf":
                if int(seqno) == 0:
                    req['id'] = 12
                else:
                    req['id'] = 14
            elif vnf_to == "amf" and vnf_from == "smf":
                req['id'] = 13
...
csv_file = folder + '/' + 'table-verify.csv'  
